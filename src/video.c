...
void video_draw_rect(u32 x, u32 y, u32 color) {
    ...
    for (int i = 0; i < x; i++) {
        for (int j = 0; j < y; j++) {
            ...
            u8 b = (color & 0xFF00) >> 8;
            u8 a = color & 0xFF;
            frame_buffer[j*pixel_offset] = r;
            frame_buffer[j*pixel_offset + 1] = g;
            frame_buffer[j*pixel_offset + 2] = b;
        }
        frame_buffer+=fb_req.pitch.pitch;
    }
}
...
void video_draw_char(char c, u32 pos_x, u32 pos_y) {
    for (int y=0; y<font_get_height(); y++) {
        for (int x=0; x<font_get_width(); x++) {
            u8 yes = font_get_pixel(c, x, y); //gets whether there is a pixel for the font at this pos...
